...
phaselock/
  README.md
  web/
    index.html            Home (landing, self-contained)
    torusnet.html         Reasoning agent  (+ torusnet.js)
    fusioncore.html       Control agent    (+ fusioncore.js)
    style.css             Shared control-room theme
  torusnet/               Python logic — mesh, routing, embedding, selector,
                          consensus, runner, server, benchmark, agent, …
  fusioncorenet/          Python logic — controller (stability + repairs)
  run_server.py           Serves web/ + the /api endpoints
  run_cli.py              Command-line run
  test_torusnet.py        Tests
```

The browser app is fully standalone (open `web/index.html` or host the `web/`
folder). The Python package is the reference backend the JS logic mirrors — run
`python run_server.py` and open `http://localhost:8000` to drive the web app
